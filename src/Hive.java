...
// PRINT LOCATION NAME AND LOCATION ID
        ...
        boolean thereIsLocationId = false;
        do {
            System.out.println("Please enter location Id");
            hive.setLocationId(methods.validatorForIntegers(scanner));
            for (int eachVariable : dbConnection.getLocationsIds()) {
                if (hive.getLocationId() == eachVariable) {
                    thereIsLocationId = true;
                    break;
                } else {
                    thereIsLocationId = false;
                }
            }            if (thereIsLocationId == false) {
                System.out.println("There is no location with this id");
            }
        }
            while (thereIsLocationId == false) ;
