...
public class DbConnection {
    ...
    public DbConnection() {
        ...
        try {
            String dbUrl = "jdbc:sqlite:bee_project1.db";
...
//                CREATE TABLE HIVES
                sqlStatement =
                        "CREATE TABLE IF NOT EXISTS hives" +
                                ...
                                "hive_status TEXT DEFAULT Brīvs," +
                                "hive_type TEXT," +
                                "hive_notes TEXT," +
                                "location_id INTEGER )";
//                "hive_notes TEXT )";
    ...
    public void createHive(Hive hive) {
        try {
            sqlStatement = "INSERT INTO hives" +
                    " (hive_id, hive_type, hive_notes, Location_id)" +
                    " VALUES (" +
                    "'" + hive.getHiveId() + "'," +
//                    "'" + hive.getHiveStatus() + "'," +
                    "'" + hive.getHiveType() + "'," +
    ...
    public void getInformationFromHistoryTable(History history) {
        try {
            ...
            while (resultSet.next()) {
                ...
                System.out.println("Id: " + id + ", Action done: " + actionDone + ", Date and time: " + dataAndTime);
            }
...
// GET ALL ACTIONS YOU HAVE DONE IN LAST VISIT
    ...
    public void createColony(Colony colony) {
        try {
            sqlStatement = "INSERT INTO colonies (hive_id,colony_origin,queen_breed,queen_year," +
                    "supers,frames, bees,brood, honey,pollen,varroa_treatment, food_added)" +
    ...
    // SELECT HIVE YOU WOULD LIKE TO UPDATE
//    SELECT * FROM colonies
//    WHERE hive_id=6;
    public void selectHive(Colony colony) {
        try {
            sqlStatement = "SELECT * FROM colonies WHERE hive_id = '" + colony.getHiveId() + "'";
            ResultSet resultSet = statement.executeQuery(sqlStatement);
            ...
            System.out.println("Error with selecting hive" + exception);
        }
    }

    //    GET ALL DATES FROM HISTORY DATABASE

    public ArrayList<String> getDate() {
        ArrayList<String> dateList = new ArrayList<String>();

        try {
            sqlStatement = "SELECT SUBSTRING (date_and_time,1,10) AS ExtractString FROM histories ";
            ResultSet resultSet = statement.executeQuery(sqlStatement);
            while (resultSet.next()) {
                dateList.add(resultSet.getString("ExtractString"));
            }

        } catch (SQLException exception) {
            System.out.println("Error in getting all dates" + exception);
        }
        return dateList;
    }
    //    GET ALL ENTRIES FROM HISTORY DATABASE

    public void getAllEntries(History history) {
        ...
        try {
            ...
            while (resultSet.next()) {
                int id = resultSet.getInt("id");
                String action = resultSet.getString("action_done");
                String dateAndTime = resultSet.getString("date_and_time");
                System.out.println("id: " + id + " action done: " + action + "date and time: " + dateAndTime);

            }

        } catch (SQLException exception) {
            System.out.println("Error in getting all entries from history database" + exception);
        }
    }

    //    SELECT DISTINCT SUBSTRING (date_and_time,1,10) AS ExtractString FROM histories
    public void getDistinctEntriesFromHistory(History history) {

        try {
            sqlStatement = "SELECT DISTINCT SUBSTRING (date_and_time,1,10) AS ExtractString FROM histories";
            ResultSet resultSet = statement.executeQuery(sqlStatement);
            while (resultSet.next()) {

                String date = resultSet.getString("ExtractString");

                System.out.println("Actions were done on: " + date);
            }

        } catch (SQLException sqlException) {
            System.out.println("Error in get distinct entries from history" + sqlException);
        }
    }

    // UPDATE HIVE STATUS WHEN COLONY IS ADDED
    public void updateHiveStatus(Hive hive, Colony colony) {
        try {

            sqlStatement = "UPDATE hives SET hive_status = 'Aizņemts' WHERE hive_id = '" + colony.getHiveId() + "'";
            statement.execute(sqlStatement);

        } catch (SQLException exception) {
            System.out.println("Error updating hive status" + exception);
        }

    }

    //    TO FIND IF THERE IS SUCH LOCATION WHEN ADDING HIVES
//SELECT location_id FROM locations
    public ArrayList<Integer> getLocationsIds() {
        ArrayList<Integer> locationsIds = new ArrayList<Integer>();

        try {
            sqlStatement = "SELECT location_id FROM locations";
            ResultSet resultSet = statement.executeQuery(sqlStatement);
            locationsIds.add(resultSet.getInt("location_id"));
        }catch (SQLException exception){
            System.out.println("Error in getting locations: " + exception);
        }return locationsIds;
    }
}
